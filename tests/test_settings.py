...
def client(tmp_path):
    ...
    archive_movies.mkdir(parents=True, exist_ok=True)
    archive_tv.mkdir(parents=True, exist_ok=True)
...
def test_save_settings_updates_paths(client, tmp_path):
    c, _ = client
    new_dir = str(tmp_path / "new_incoming")
    resp = c.post("/api/settings", json={"active_dir": new_dir})
    assert resp.status_code == 200
    assert resp.json()["active_dir"] == new_dir

    # Persisted: a subsequent GET reflects the change.
    resp2 = c.get("/api/settings")
    assert resp2.json()["active_dir"] == new_dir
...
def test_permissions_check_reports_missing_dir(client, tmp_path):
    ...
    c.post("/api/settings", json={"active_dir": missing})
